...
$(users) : group=$(shell  sqlite3 $(db) 'select grp  from users where user="$(user)"')
...
$(users) : % : ldif/group_template.ldif ldif/user_template.ldif made/created /usr/bin/pwgen
	slapcat -a '(&(cn=$(group))(objectClass=posixGroup))' | grep dn: || sed -e s/%GROUP%/$(group)/g -e s/%GID%/$(gid)/g ldif/group_template.ldif | $(slapadd)
	slapcat -a 'uid=$(user)' | grep dn: || sed -e s/%GROUP%/$(group)/g -e s/%GID%/$(gid)/g -e s/%USER%/$(user)/g -e s/%UID%/$(uid)/g -e s:%HOME%:$(home):g -e s:%SHA_PASSWORD%:$(sha_pwd):g ldif/user_template.ldif | $(slapadd)
	mkdir -p $(home)
	chmod 0$(mod) $(home)
	chown $(uid):$(gid) $(home)
